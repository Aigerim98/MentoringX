...
class NetworkManager {
    ...
    func getPhotoTemp(id: Int, token: String, completion: @escaping (Image) -> Void) {
    ...
    func getMenteeId(token: String, completion: @escaping (MenteeIds) -> Void) {
        var components = urlComponents
        components.path = "/connections/show_connections_mentor"
        ...
        let task = session.dataTask(with: urlRequest) { data, response, error in
            ...
            do {
                let menteeIds = try JSONDecoder().decode(MenteeIds.self, from: data)

                DispatchQueue.main.async {
                    completion(menteeIds)
                }
            }catch {
                print("no json")
            }
    ...
    func postLikeToMentor(token: String, id: Int, completion: @escaping (String) -> Void){
    ...
    func postLikeToMentee(token: String, id: Int, completion: @escaping (String) -> Void){
        let queryItem = [URLQueryItem(name: "matchID", value: String(id))]
        
        var components = urlComponents
        components.path = "/connections/match_from_mentor"
        components.queryItems = queryItem
